...
(defun long-test ()
  ...
  (let ((cnt 0))
   ...
   ; (with-open-file (stream "hands.txt" :direction :output :if-exists :supersede)
    (loop for a from 0 to 45 do
	 ;(logior hand (ash 1 a))
	 (loop for b from (+ a 1) to 46 do
	      ;(logior hand (ash 1 b))
	      (loop for c from (+ b 1) to 47 do
		   ;(logior hand (ash 1 c))
		   (loop for d from (+ c 1) to 48 do
			;(logior hand (ash 1 d))
			(loop for e from (+ d 1) to 49  do
			     ;(logior hand (ash 1 e))
			     (loop for f from (+ e 1) to 50 do
				  ;(logior hand (ash 1 f))
				  (loop for g from (+ f 1) to 51 do
				       ;(logior hand (ash 1 g))
				       ;(setf hand (card-m (list a b c d e f g)))
				       (eval-hand-7 a b c d e f g)
				       (incf cnt))))))))
				      ; (logand hand (lognot g)))
				  ;(logand hand (lognot f)))
			     ;(logand  hand (lognot e)))
			;(logand hand (lognot d)))
		   ;(logand  hand (lognot c)))
	      ;(logand hand (lognot b)))
	 ;(logand  hand (lognot a)))
    cnt))
...
(defun short-test ()
  ...
  (let ((hand 0))
    (loop for a from 0 to 10 do
	 (logior hand (ash 1 a))
	 (loop for b from (+ a 1) to 11 do
	      (logior hand (ash 1 b))
	      (loop for c from (+ b 1) to 12 do
		   (logior hand (ash 1 c))
		   (loop for d from (+ c 1) to 13 do
			(logior hand (ash 1 d))
			(loop for e from (+ d 1) to 14  do
			     (logior hand (ash 1 e))
			     (loop for f from (+ e 1) to 15 do
				  (logior hand (ash 1 f))
				  (loop for g from (+ f 1) to 16 do
				       (logior hand (ash 1 g))
				       (eval-hand-7 a b c d e f g)
				       (logand hand (lognot g)))
				  (logand hand (lognot f)))
...
(defun profile-eval-hand ()
  (sb-profile:reset)
  (sb-profile:profile eval-hand-7)
  (long-test)
  (sb-profile:report))
